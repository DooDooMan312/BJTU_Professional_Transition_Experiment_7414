...
def create_model():
    """创建并返回模型"""
    model = models.resnet50(pretrained=pretrain)  # 修改为resnet50
    ...
    # 修改最后一层全连接层（ResNet50原始输出是2048维）
    model.fc = nn.Linear(2048, n_classes)  # 修改输入维度为2048
...
def save_loss_acc_to_csv(epoch, train_loss, train_acc, val_loss, val_acc, elapsed_time, model_save):
    """保存训练指标到CSV"""
    file_path = os.path.join(model_save, 'training_metrics.csv')
...
def main():
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(model_save_path, 'best_resnet50_model.pth'))  # 修改保存文件名
    ...
    # 最终测试
    model.load_state_dict(torch.load(os.path.join(model_save_path, 'best_resnet50_model.pth')))
    test_loss, test_acc, test_labels, test_preds = evaluate(model, test_loader, criterion)

    # 生成混淆矩阵
    cm = confusion_matrix(test_labels, test_preds)
    classes = ['Wind', 'Man-made', 'Excavation']
    plot_confusion_matrix(cm, 'resnet50', classes, normalize=False)
