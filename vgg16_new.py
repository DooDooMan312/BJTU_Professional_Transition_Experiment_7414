...
def create_model():
    ...
    # 修改最后一层全连接层（VGG16原始输出是1000类）
    model.classifier[6] = nn.Linear(4096, n_classes)  # VGG16最后全连接层输入是4096
...
def main():
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(model_save_path, 'best_vgg16_model.pth'))
    ...
    # 最终测试
    model.load_state_dict(torch.load(os.path.join(model_save_path, 'best_vgg16_model.pth')))
    ...
    plot_confusion_matrix(cm, 'VGG16',classes, normalize=False)
